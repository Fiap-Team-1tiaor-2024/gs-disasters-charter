...
def detectar_anomalias(df_acumulados: pd.DataFrame,
    ...
    if 'estacao' not in df_result.columns and df_result.index.name == 'estacao':
        df_result = df_result.reset_index(drop=False)
    if 'estacao' not in df_result.columns and 'estacao' in df_result.index.names:
        df_result = df_result.reset_index(drop=False)
    ...
    cols_top = ['estacao', 'prec_acum_24h', 'score_anomalia']
    if 'municipio' in df_result.columns:
        cols_top.insert(1, 'municipio')
    cols_top = [c for c in cols_top if c in df_result.columns]
    top = df_result.nsmallest(10, 'score_anomalia')[cols_top]
    print(f"\n  Top-10 eventos mais anômalos:")
    print(top.to_string())
...
def criar_features_preditivas(df_acumulados: pd.DataFrame,
    ...
    y_futuro = y_atual.groupby(df['estacao']).shift(-horas_ahead)
...
def treinar_modelo_preditivo(df_acumulados: pd.DataFrame,
                              horas_ahead: int = 3,
                              caminho_outputs: str = 'data/ml/images') -> dict:
    """
...
def treinar_e_salvar(df_acumulados: pd.DataFrame,
                     ...
                     caminho_outputs: str = 'data/ml/images',
